...
//
// The time in seconds to run the benchmark.
// Can be changed using the "-time" command line parameter.
//
int64_t MaxTime = 30;

//
// The number of connection established in the given time.
    ...
    )
{
    ...
    if (GetFlag(argc, argv, "help") || GetFlag(argc, argv, "?")) {
        ...
        time_t Finish = time(NULL) + MaxTime;
        ...
        double totalTime = (Stop.tv_sec - Start.tv_sec) + (Stop.tv_usec - Start.tv_usec) / 1000000.0;
        printf ("%d connections in %.2fs; %.2f connections/user sec\n", NumConnections, totalTime, ((double)NumConnections / totalTime));
        printf ("%d connections in %ld real seconds\n", NumConnections, time(NULL) - Finish + MaxTime);
    }
